...
#include <string.h>
	...
	inv_error_t inv_create_mutex(uint32 *mutex);
	inv_error_t inv_lock_mutex(uint32 mutex);
	inv_error_t inv_unlock_mutex(uint32 mutex);
	FILE *inv_fopen(char *filename);
	void inv_fclose(FILE *fp);

	inv_error_t inv_destroy_mutex(uint32 handle);
